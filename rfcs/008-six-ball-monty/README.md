...
2. **Solution:** A new `sixballmonty-mvp` workspace built on a **pure, deterministic, tick-based engine** (immutable state, seeded RNG, input-event driven). Everything above the engine — controls, local 2P, online netcode, puzzle maker — layers on without touching core rules.
...
4. **Effort:** Phased. Phase 1 (headless engine incl. versus core, zero graphics) ~3 days; graphics start in Phase 2; multiplayer (local ph3, online ph4) ships before the puzzle maker (ph5).
...
**Estimated effort:** Phase 1 (headless engine): ~3 days; Phases 2-5 each 2-5 days, independently shippable
